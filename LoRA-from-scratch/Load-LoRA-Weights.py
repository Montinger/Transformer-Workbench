...
def initialize_model(task, model_id, nr_classes):
    if task in glue_task_list:
        return LoraWrapperRoberta(model_id=model_id, task_type="glue", num_classes=nr_classes, lora_rank=LORA_RANK)
    else:
        return LoraWrapperRoberta(model_id=model_id, task_type=task, lora_rank=LORA_RANK)
...
for task in train_task_list:
    ...
    if not weights_input_path.is_file():
        print(f"\nDid not found weights for task in location {weights_input_path}. Skipping.")
        continue
    model = LoraWrapperRoberta.load_lora_state_dict(weights_input_path)
...
df_res_collector.to_csv(io_path / "load_lora_test_results.csv")
