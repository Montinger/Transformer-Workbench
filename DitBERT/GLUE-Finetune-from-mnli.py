...
model_path = pathlib.Path("GLUE-Outputs/run-03/model_mnli.pth")
...
run_glue_tasks = ["cola", "mrpc", "rte", "stsb", "wnli"]
...
for glue_task in run_glue_tasks:
    ...
    class DitBERT_Classifier(nn.Module):
        def __init__(self, base_model, num_classes, dropout_rate=0.1, add_layer_norm=True):
    ...
    mnli_model = torch.load(model_path, map_location=torch.device(device))

    # Instantiate your new model with your base model and the number of classes
    model = DitBERT_Classifier(base_model=mnli_model.base_model, num_classes=nr_classes, add_layer_norm=add_layer_norm)
